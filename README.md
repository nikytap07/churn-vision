...
📸 _![Предсказания1](./docs/screenshots/prediction_input.png)_
...
📸 _![Предсказания2](./docs/screenshots/prediction_output.png)_

## 🧬 Кластеризация клиентов (`clustering.py`)

Модуль `clustering.py` реализует функциональность **кластерного анализа** пользователей на основе контрактных и поведенческих признаков.

### 🔹 KMeans-кластеризация

- Автоматический подбор числа кластеров с помощью **метода локтя**;
- Оценка качества разбиения по **Silhouette Score**;
- Визуализация кластеров с помощью **UMAP-проекции**;
- Интерактивный выбор признаков и просмотр их распределения по кластерам.

### 🔸 Иерархическая кластеризация

- Построение **дендрограммы** на основе алгоритма агломеративной кластеризации;
- Возможность **интерактивной обрезки** дерева на `K` кластеров;
- Визуализация распределения признаков внутри кластеров с помощью **boxplot**.

### 🖼️ Примеры визуализаций

| Визуализация | Описание |
|-------------|----------|
| ![UMAP](./docs/screenshots/Elbow.png) | Метод локтя |
| ![UMAP](./docs/screenshots/UMAP.png) | UMAP-проекция кластеров |
| ![Boxplot Lifetime](./docs/screenshots/cl_dist.png) | Распределение признаков по кластерам |
| ![Дендрограмма](./docs/screenshots/dendrogram.png) | Иерархическая дендрограмма с линиями для K=2,4,5 |

---

### 🧭 Интерфейс вкладки **👥 Кластеризация**

Пользователь может:

1. **Выбрать количество кластеров (K)** для KMeans через слайдер;
2. Просмотреть:
   - количество клиентов в каждом кластере;
   - долю `churn` по каждому кластеру;
   - UMAP-график кластеров;
3. Выбрать кластер и признаки для анализа распределений;
4. Провести иерархическую кластеризацию и разрезать дерево на `K` кластеров с построением **boxplot-графика** по выбранному признаку.
